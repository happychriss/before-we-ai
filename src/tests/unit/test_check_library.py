...
def run(store, con, template, params, tolerances=None):
    return run_check(store, con, CheckPlan(template=template, params=params), tolerances)


class TestVerdictFunctions:
    def test_empty_expected(self):
        assert empty_expected([], [], {}).verdict is CheckVerdict.PASS
        assert empty_expected([(1,)], [], {}).verdict is CheckVerdict.FAIL

    def test_report_only_never_fails(self):
        # K6: legitimate orphans are a finding, structurally never FAIL.
        assert report_only([], [], {}).verdict is CheckVerdict.PASS
        assert report_only([(1,)] * 500, [], {}).verdict is CheckVerdict.INCONCLUSIVE

    def test_anti_join_dispatch(self):
        assert anti_join_verdict([(1,)], [], {"expectation": "empty"}).verdict is CheckVerdict.FAIL
        assert (
            anti_join_verdict([(1,)], [], {"expectation": "report"}).verdict
            is CheckVerdict.INCONCLUSIVE
        )

    def test_coverage_grades(self):
        ctx = {"expected_count": 4}
        assert coverage_verdict([], [], ctx).verdict is CheckVerdict.PASS
        assert coverage_verdict([(1,)], [], ctx).verdict is CheckVerdict.INCONCLUSIVE
        assert coverage_verdict([(1,)] * 4, [], ctx).verdict is CheckVerdict.FAIL
    ...
    def test_cardinality_thresholds(self):
        ...
        assert good.verdict is CheckVerdict.PASS
        fanout = cardinality_verdict([(2, 600, 4000, 2)], cols, ctx)  # T6 shape
        assert fanout.verdict is CheckVerdict.FAIL
...
class TestCheckRuns:
    def test_anti_join_fail_and_artifacts(self, store, con):
        ...
        assert record.check_plan_id in store.checks  # check persisted with the run
    ...
    def test_report_expectation_drafts_question(self, store, con):
        ...
        # Re-running the same check shape does not duplicate the clarification question.
    ...
    def test_duplicate_and_grain(self, store, con):
        ...
        assert dup.verdict is CheckVerdict.FAIL  # '9' twice
        clean = run(store, con, "grain", {"table": "parents", "key_columns": ["id"]})
        assert clean.verdict is CheckVerdict.PASS
    ...
    def test_coverage(self, store, con):
        ...
        assert partial.verdict is CheckVerdict.INCONCLUSIVE
    ...
    def test_tolerance_override_comes_from_config_only(self, store, con):
        ...
        assert run(store, con, "reconciliation", params).verdict is CheckVerdict.FAIL
        tolerant = run(
            store, con, "reconciliation", params,
            tolerances={"reconciliation": {"absolute": 50.0}},
        )
        assert tolerant.verdict is CheckVerdict.PASS

    def test_unknown_template_raises(self, store, con):
        with pytest.raises(ValueError, match="unknown check definition"):
