...
**`model` — der epistemische Kern.** Pydantic-Modelle (Source, DataProfile, Claim, EvidenceRecord mit den fünf Evidenztypen, CheckPlan, ClarificationQuestion, ConceptClaim, **MappingClaim**) plus Zustandsmaschine und Promotionsregeln als reine Funktionen. Keine IO-Abhängigkeit. Vollständig unit-testbar. Hier lebt: Konflikt erzwingt `unresolved`, nur Sonde/Mensch promoted, KI erzeugt nur `proposed`. Claims können **Abhängigkeiten** deklarieren: Eine Sonde läuft erst, wenn vorausgesetzte Claims mindestens `test-supported` sind (z. B. Nebenbuch=Hauptbuch erst nach Bindung beider Seiten). Auflösung per topologischer Sortierung — ein Mini-Scheduler, kein Workflow-System.
